...
public class SudokuHighlightSystem : MonoBehaviour
{
    ...
    int SIZE => SudokuRules.Size;//Constante que representa el tamaño del Sudoku.
        ...
        //recibe SudokuCell cell que es La celda que fue presionada.
    {
        ...
        int index = SudokuRules.GetCellIndex(r, c);//Convierte fila/columna a índice lineal.
        //Ejemplo:
        //r = 2
        //c = 5
        //index = 2 * 9 + 5
        ...
        //recibe int col que es la columna seleccionada
    {
        ...
        //Luego calcula dónde empieza la caja:
        int startRow = (row / SudokuRules.BoxRows) * SudokuRules.BoxRows;
        int startCol = (col / SudokuRules.BoxCols) * SudokuRules.BoxCols;
        for (int r = 0; r < SudokuRules.BoxRows; r++)//Recorre las filas internas de la caja.
            for (int c = 0; c < SudokuRules.BoxCols; c++)//Recorre las columnas internas de la caja.
                board[startRow + r, startCol + c].SetHighlight(highlightColor);//pinta toda la caja
        ...
        //recibe un int number que es El número que se quiere buscar.
    {
        ...
        for (int i = 0; i < SudokuRules.CellCount; i++)//Recorre las celdas del Sudoku.
        {
            if (data.values[i] == number)//pregunta ¿Esta celda tiene el número que estoy buscando?
            {
                ...
                //si i = 23
                int r = SudokuRules.GetRow(i);//r = 23 / SIZE = 2
                int c = SudokuRules.GetCol(i);//c = 23 % SIZE = 5
        ...
        //recibe int number : Número que se quiere revisar.
    {
        ...
        for (int c = 0; c < SIZE; c++)//Recorre todas las columnas de esa misma fila.
        {
            ...
            ////row = 5
            ////col = 7
            int index = SudokuRules.GetCellIndex(row, c);//int index = 5 * SIZE + 7 = (5 * SIZE) + 7
        ...
        for (int r = 0; r < SIZE; r++)//Aquí recorre todas las filas de la misma columna
        {
            ...
            ////row = 5
            ////col = 7
            int index = SudokuRules.GetCellIndex(r, col);//int index = 5 * SIZE + 7
        ...
        //Luego revisa la caja
        int startRow = (row / SudokuRules.BoxRows) * SudokuRules.BoxRows;
        int startCol = (col / SudokuRules.BoxCols) * SudokuRules.BoxCols;
 
        //luego Recorre las celdas de esa caja
        for (int r = 0; r < SudokuRules.BoxRows; r++)//revisa las filas de la caja
            for (int c = 0; c < SudokuRules.BoxCols; c++)//revisa las columnas de la caja
            {
                ...
                //Convierte fila/columna a índice lineal
                int index = SudokuRules.GetCellIndex(rr, cc);
    ...
    public void SetNotesButtonImage(Image image)//Reasigna la imagen del botón de notas. Lo usa SudokuGameUI.
    {
        notesButtonImage = image;
    }
        ...
        //recibe Color color : Color que se quiere aplicar.
    {
        ...
        int r = SudokuRules.GetRow(index);//r = 23 / SIZE = 2
        //Luego convierte índice a columna
        int c = SudokuRules.GetCol(index);//c = 23 % SIZE = 5
